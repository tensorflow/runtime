...
void RotateRight(llvm::MutableArrayRef<ssize_t> array, size_t k = 1);
...
struct WindowedOutputData {
  llvm::SmallVector<ssize_t, 4> output_dims;  // NCHW
  llvm::SmallVector<ssize_t, 2> strides;
  llvm::SmallVector<ssize_t, 2> dilations;
  llvm::SmallVector<ssize_t, 2> paddings_before;
  llvm::SmallVector<ssize_t, 2> paddings_after;
};
...
Expected<TensorMetadata> TfPadOutputShape(
  ...
  llvm::SmallVector<ssize_t, 8> output_shape;
  for (int d = 0; d < input_shape.GetRank(); ++d) {
    ...
    const ssize_t size_d = input_shape.GetDimensionSize(d);
