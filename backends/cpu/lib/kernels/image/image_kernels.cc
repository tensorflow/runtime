...
static AsyncValueRef<DenseHostTensor> DecodeJpeg(const std::string& data,
                                                 Location loc) {
  auto host = loc.GetHost();
  if (!llvm::StringRef(data).startswith("\xff\xd8\xff")) {
    return loc.EmitErrorAsync("image does not have jpeg format");
  }
  ...
  jpeg::Uncompress(
      ...
      [host, loc, &result](int width, int height, int channels) -> uint8_t* {
        auto tensor = DenseHostTensor::CreateUninitialized<uint8_t>(
            TensorShape({height, width, channels}), host);
        if (!tensor) {
          result.EmitError(loc, "cannot allocate tensor");
          return nullptr;
        }
...
static llvm::Expected<DenseHostTensor> ResizeBilinear(
    const DenseHostTensor& input, int64_t height, int64_t width, Location loc) {
  auto host = loc.GetHost();
