...
func @unary_ops() attributes {tfrt.sync} {
  ...
  // CHECK: shape = [5], values = [0.000000e+00, 9.162908e-01, 1.098612e+00, 1.504077e+00, 1.609438e+00]
  tfrt_dht_sync.print_tensor %b

  "tf_sync.Log1p.f32"(%a, %b) : (!t.tensor, !t.tensor)->()

  // CHECK: shape = [5], values = [6.931472e-01, 1.252763e+00, 1.386294e+00, 1.704748e+00, 1.791759e+00]
  tfrt_dht_sync.print_tensor %b
  ...
  // CHECK: shape = [5], values = [7.071067e-01, 6.324555e-01, 5.773503e-01, 4.714045e-01, 4.472136e-01]
  tfrt_dht_sync.print_tensor %b
  ...
  // CHECK: shape = [5], values = [7.310586e-01, 9.241418e-01, 9.525741e-01, 9.890131e-01, 9.975274e-01]
  tfrt_dht_sync.print_tensor %b
