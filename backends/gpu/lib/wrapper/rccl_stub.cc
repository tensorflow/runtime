...
// Implementation of the RCCL API forwarding calls to symbols dynamically loaded
// from the real library.
#include "tfrt/gpu/wrapper/rccl_stub.h"

#include "symbol_loader.h"

// Memoizes load of the .so for this RCCL library.
static void *LoadSymbol(const char *symbol_name) {
  static SymbolLoader loader("librccl.so");
...
static ncclResult_t DynamicCall(const char *symbol_name, Args &&...args) {
  static auto func_ptr = GetFunctionPointer<Func>(symbol_name);
  if (!func_ptr) return ncclSystemError;
  return func_ptr(std::forward<Args>(args)...);
}

extern "C" {
#include "rccl_stub.cc.inc"
}  // extern "C"
