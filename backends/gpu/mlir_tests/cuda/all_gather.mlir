...
func.func @all_gather(
  ...
  %ch3 = tfrt_gpu.ccl.all_gather %ccl, %pinned_in, %pinned_out, ncclInt32, %ch2
...
// CHECK-LABEL: --- Running 'all_gather_test'
func.func @all_gather_test() {
  ...
  %ch0, %t0 = tfrt.call @all_gather(%rank0, %count, %id)
              : (i32, i32, !tfrt_gpu.ccl.id) -> (!tfrt.chain, !t.tensor)
  %ch1, %t1 = tfrt.call @all_gather(%rank1, %count, %id)
              : (i32, i32, !tfrt_gpu.ccl.id) -> (!tfrt.chain, !t.tensor)
