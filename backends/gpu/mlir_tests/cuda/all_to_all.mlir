...
func.func @all_to_all(
  ...
  %tensor_out = tfrt_dht.create_uninitialized_tensor.i32.1 [2: i64]
  %ch2 = tfrt_dht.set_tensor_with_constant_values.i32 %tensor_out, %ch1 [0: i32, 0: i32]
  ...
  %ch3 = tfrt_gpu.ccl.all_to_all %ccl, %pinned_in, %pinned_out, ncclInt32, %ch2
...
// CHECK-LABEL: --- Running 'all_to_all_test'
func.func @all_to_all_test() {
  ...
  %ch0, %t0 = tfrt.call @all_to_all(%rank0, %count, %id)
              : (i32, i32, !tfrt_gpu.ccl.id) -> (!tfrt.chain, !t.tensor)
  %ch1, %t1 = tfrt.call @all_to_all(%rank1, %count, %id)
              : (i32, i32, !tfrt_gpu.ccl.id) -> (!tfrt.chain, !t.tensor)

  // CHECK: DenseHostTensor dtype = i32, shape = [2], values = [1, 1]
  %ch2 = tfrt_dht.print_tensor %t0, %ch0
  %ch3 = tfrt.merge.chains %ch1, %ch2 : !tfrt.chain, !tfrt.chain
  // CHECK: DenseHostTensor dtype = i32, shape = [2], values = [2, 2]
