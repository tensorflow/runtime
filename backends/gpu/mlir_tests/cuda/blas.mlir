...
func @create_two_tensors_on_gpu(%context: !tfrt_gpu.context, %allocator: !tfrt_gpu.allocator, %stream: !tfrt_gpu.stream) -> (!tfrt_gpu.buffer, !tfrt_gpu.buffer) {
...
func @blas_axpy() {
  ...
  %gpu_buffer_0, %gpu_buffer_1 = tfrt.call @create_two_tensors_on_gpu(%context, %allocator, %stream) : (!tfrt_gpu.context, !tfrt_gpu.allocator, !tfrt_gpu.stream) -> (!tfrt_gpu.buffer, !tfrt_gpu.buffer)
  ...
  %ch4 = tfrt_gpu.mem.copy_device_to_host %context, %result_buffer, %gpu_buffer_1, %buffer_size_in_bytes, %stream, %ch3
...
func @blas_gemm() {
  ...
  %gpu_buffer_A, %gpu_buffer_B = tfrt.call @create_two_tensors_on_gpu(%context, %allocator, %stream) : (!tfrt_gpu.context, !tfrt_gpu.allocator, !tfrt_gpu.stream) -> (!tfrt_gpu.buffer, !tfrt_gpu.buffer)
  ...
  %gpu_buffer_C, %ch3 = tfrt_gpu_test.copy_tensor_host_to_device %context, %allocator, %stream, %tensor_C, %ch2

  %dim = tfrt.constant.i32 2
  %alpha = tfrt.constant.f32 1.0
  %beta = tfrt.constant.f32 1.0
  %ch4 = tfrt_gpu.blas.gemm.f32 %blas, %dim, %dim, %dim, %alpha, %gpu_buffer_A, %dim, %gpu_buffer_B, %dim, %beta, %gpu_buffer_C, %dim, %ch3 { transa = false, transb = false }
  ...
  %buffer_size_in_bytes = tfrt.constant.i64 16 // [2, 2] * 4 bytes floats = 16 bytes
  %ch6 = tfrt_gpu.mem.copy_device_to_host %context, %result_buffer, %gpu_buffer_C, %buffer_size_in_bytes, %stream, %ch5
  // CHECK: DenseHostTensor dtype = F32, shape = [2, 2]
...
func @blas_gemm_ex() {
  ...
  %alpha = tfrt.constant.f32 1.0
  %beta = tfrt.constant.f32 1.0
  %ch4 = tfrt_gpu.blas.gemm.ex %blas, %dim, %dim, %dim, %alpha, %gpu_buffer_A, %type, %dim, %gpu_buffer_B, %type, %dim, %beta, %gpu_buffer_C, %type, %dim, %type, %algo, %ch3 { transa = false, transb = false }
...
func @blas_gemm_strided_batched_ex() {
  ...
  %ch4 = tfrt_gpu.blas.gemm.strided.batched.ex %blas, %dim, %dim, %dim, %alpha, %gpu_buffer_A, %type, %dim, %stride, %gpu_buffer_B, %type, %dim, %stride, %beta, %gpu_buffer_C, %type, %dim, %stride, %batch_count, %type, %algo, %ch3 { transa = false, transb = false }
