...
func @create_two_tensors_on_gpu(%context: !cuda.context, %allocator: !cuda.allocator, %stream: !cuda.stream, %chain: !tfrt.chain) -> (!cuda.buffer, !cuda.buffer, !tfrt.chain) {
  ...
  %gpu_buffer_0, %ch2 = cuda_test.copy_tensor_host_to_device %context, %allocator, %stream, %tensor_0, %ch1
  ...
  %gpu_buffer_1, %ch4 = cuda_test.copy_tensor_host_to_device %context, %allocator, %stream, %tensor_1, %ch3

  %ch_out = tfrt.merge.chains %ch2, %ch4

  tfrt.return %gpu_buffer_0, %gpu_buffer_1, %ch_out : !cuda.buffer, !cuda.buffer, !tfrt.chain
}
...
func @cublas_axpy() -> !tfrt.chain {
  ...
  %gpu_buffer_0, %gpu_buffer_1, %cha1 = tfrt.call @create_two_tensors_on_gpu(%context, %allocator, %stream, %cha0) : (!cuda.context, !cuda.allocator, !cuda.stream, !tfrt.chain) -> (!cuda.buffer, !cuda.buffer, !tfrt.chain)
  ...
  %chc2 = cuda.mem.copy_device_to_host %context, %result_buffer, %gpu_buffer_1, %buffer_size_in_bytes, %stream, %chc1
...
func @cublas_gemm() -> !tfrt.chain {
  ...
  %gpu_buffer_A, %gpu_buffer_B, %cha0 = tfrt.call @create_two_tensors_on_gpu(%context, %allocator, %stream, %ch6) : (!cuda.context, !cuda.allocator, !cuda.stream, !tfrt.chain) -> (!cuda.buffer, !cuda.buffer, !tfrt.chain)
  ...
  %gpu_buffer_C, %cha2 = cuda_test.copy_tensor_host_to_device %context, %allocator, %stream, %tensor_C, %cha1

  %no_transpose = tfrt.constant.i32 0
  %dim = tfrt.constant.i32 2
  %alpha = tfrt.constant.f32 1.0
  %beta = tfrt.constant.f32 1.0
  %cha3 = cuda.blas.gemm.f32 %context, %cublas_handle, %no_transpose, %no_transpose, %dim, %dim, %dim, %alpha, %gpu_buffer_A, %dim, %gpu_buffer_B, %dim, %beta, %gpu_buffer_C, %dim, %cha2
  ...
  %buffer_size_in_bytes = tfrt.constant.i64 16 // [2, 2] * 4 bytes floats = 16 bytes
  %chb2 = cuda.mem.copy_device_to_host %context, %result_buffer, %gpu_buffer_C, %buffer_size_in_bytes, %stream, %chb1
  // CHECK: DenseHostTensor dtype = F32, shape = [2, 2]
  ...
  %chc1 = cuda.allocator.destroy %allocator, %chc0
