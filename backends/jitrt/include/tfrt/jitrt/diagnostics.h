...
#ifndef TFRT_BACKENDS_JITRT_INCLUDE_TFRT_JITRT_DIAGNOSTICS_H_
#define TFRT_BACKENDS_JITRT_INCLUDE_TFRT_JITRT_DIAGNOSTICS_H_
...
// JitRt runtime diagnostics borrows a lot of ideas from the MLIR compile time
...
class InFlightDiagnostic {
 public:
  ...
  // Example:
  //
  //   mlir::LogicalResult call(DiagnosticEngine diag, ...) {
  //     if (<check failed>) return diag.EmitError() << "Oops";
  //     ...
  //   }
  //
  operator mlir::LogicalResult() const { return mlir::failure(); }  // NOLINT
...
//
// JitRt users must set up diagnostic engine to report errors back to the
...
class DiagnosticEngine {
 public:
  ...
  using HandlerTy = std::function<mlir::LogicalResult(Diagnostic &)>;
  ...
  void Emit(Diagnostic diagnostic) const {
    for (auto &handler : llvm::reverse(handlers_))
      if (mlir::succeeded(handler(diagnostic))) return;
...
#endif  // TFRT_BACKENDS_JITRT_INCLUDE_TFRT_JITRT_DIAGNOSTICS_H_
