...
TEST_F(BefAttrEmitterTest, EmitAggregateAttribute) {
  ...
  EXPECT_EQ(attr.GetElement<Attribute<int32_t>>(0).get(), kTestAggregateAttr1);

  EXPECT_EQ(attr.GetElementType(1),
            static_cast<BEFAttributeType>(DType::String));
  EXPECT_EQ(attr.GetElement<StringAttribute>(1).get(), kTestAggregateAttr2);

  EXPECT_EQ(attr.GetElementType(2), static_cast<BEFAttributeType>(DType::F32));
  EXPECT_EQ(attr.GetElement<Attribute<float>>(2).get(), kTestAggregateAttr3);
}
