...
TEST(BefEncodingTest, IsValidAlignmentWith0) {
  EXPECT_FALSE(IsValidAlignment(0));
}

TEST(BefEncodingTest, IsValidAlignmentWith1) {
  EXPECT_TRUE(IsValidAlignment(1));
}

TEST(BefEncodingTest, IsValidAlignmentWith2) {
  EXPECT_TRUE(IsValidAlignment(2));
}

TEST(BefEncodingTest, IsValidAlignmentWith3) {
  EXPECT_FALSE(IsValidAlignment(3));
}

TEST(BefEncodingTest, IsValidAlignmentWith4) {
  EXPECT_TRUE(IsValidAlignment(4));
}

constexpr unsigned kTestMaxAlignment = 64;
TEST(BefEncodingTest, CalculateAlignmentPaddingSize) {
