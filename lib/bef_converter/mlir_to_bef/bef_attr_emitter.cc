...
size_t BefAttrEmitter::EmitDenseAttribute(BEFAttributeType attribute_type,
  ...
  const size_t offset = EncodeDenseAttrHeader(
      element_type, shaped_type.getShape(),
      GetAttributeDataTypeByteSize(attribute_type) * element_count);
...
size_t BefAttrEmitter::EmitAggregatedAttribute(mlir::ArrayAttr attr) {
  ...
  const size_t offset =
      EncodeAggregatedAttrHeader(element_count, &type_offset, &offset_offset);
  for (auto element : attr) {
    EncodeAggregatedAttrEntryTypeAndOffset(&type_offset, &offset_offset,
                                           GetBefAttributeType(element),
                                           EmitAttribute(element) - offset);
  }
  EncodeAggregatedAttrLength(&offset_offset, offset);

  assert(GetRequiredAlignment() <= 8 &&
         "The AggregateAttr instance cannot be trivially copied when there is "
         "an element having an alignment constrant bigger than 8.");
  return offset;
}
