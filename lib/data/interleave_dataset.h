...
// input dataset to construct intermediate iterators. Then it interleaves the
// values from those intermediate iterators and output those values to construct
// intermediate iterators, then interleaves the values generated by those
// iterators. The user-defined function is expected to return a single value of
// type Dataset.
//
...
class InterleaveDatasetIterator : public Iterator {
 ...
 private:
  ...
  //
  // It repeats the following steps until it encounters a value with unavailable
  // eof or when output_buffer_ is empty:
