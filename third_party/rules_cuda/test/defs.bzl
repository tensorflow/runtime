...
load("//cuda:defs.bzl", "CudaInfo")
load("@bazel_skylib//lib:unittest.bzl", "analysistest", "asserts")

def _cuda_targets_test_impl(ctx):
    env = analysistest.begin(ctx)
    target_under_test = analysistest.target_under_test(env)
    asserts.equals(env, ["sm_70", "sm_75"], target_under_test[CudaInfo].value)
...
def _cuda_runtime_test_impl(ctx):
    env = analysistest.begin(ctx)
    target_under_test = analysistest.target_under_test(env)
    asserts.equals(env, "<merged target //test:cuda_test_runtime>", str(target_under_test))
    return analysistest.end(env)
...
def _cuda_library_test_impl(ctx):
    ...
    asserts.equals(env, ["test/cuda.h"], [h.short_path for h in headers])
    return analysistest.end(env)
